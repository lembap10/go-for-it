...
function addAttendees(numAdults, numKids, numDogs) {
  if (numAdults != 0)
    trips[trips.length - 1].attendees += `${numAdults} adults`;
  if (numAdults != 0) trips[trips.length - 1].attendees += `${numKids} kids`;
  if (numAdults != 0) trips[trips.length - 1].attendees += `${numDogs} dogs`;
}
